...
export function PrintFichas({ fichas, config }: { fichas: Ficha[]; config: Configuracoes }) {
  ...
  return (
    <div className="hidden print:block w-full bg-white text-black text-sm absolute top-0 left-0 z-50">
      <style type="text/css" media="print">
        {`
          @page { size: A4; margin: 15mm; }
          body { -webkit-print-color-adjust: exact; print-color-adjust: exact; background: white; }
          .ficha-print-page { break-after: page; page-break-after: always; position: relative; min-height: 90vh; }
          .ficha-print-page:last-child { break-after: auto; page-break-after: auto; }
          .print-footer { position: absolute; bottom: 0; left: 0; right: 0; font-size: 10px; border-top: 1px solid #000; padding-top: 4px; display: flex; justify-content: space-between; }
        `}
      </style>

      {fichas.map((ficha) => (
        <div key={ficha.id} className="ficha-print-page pb-12">
          <div className="border border-black p-3 mb-4 text-center">
            <h1 className="text-xl font-bold uppercase mb-1">Ficha de Recebimento de Amostras</h1>
          ...
          <div className="mb-2 font-bold uppercase text-xs">Itens da Amostra</div>
          <div className="space-y-3">
            {ficha.itens.map((item, idx) => (
              <div key={item.id} className="border border-black p-2 text-xs break-inside-avoid">
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                  ...
                  <div>
                    <span className="font-semibold">Setor Destino:</span> {item.setorDestino}
                  </div>
                  <div className="col-span-2">
                  ...
                  {item.ordemServico && (
                    <div>
                      <span className="font-semibold">Ordem de Serviço:</span> {item.ordemServico}
                    </div>
                  )}
                  {item.protocoloWeb && (
                    <div>
                  ...
                  {item.fatorDiluicao && (
                    <div>
                      <span className="font-semibold">Fator de Diluição:</span> {item.fatorDiluicao}
                    </div>
                  )}
                  {item.enviou1gAtivo && (
                    <div>
